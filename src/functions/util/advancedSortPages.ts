...
async function asyncSort<T>(
  array: T[],
  asyncComparator: (a: T, b: T) => Promise<number>,
): Promise<T[]> {
...
export default new NativeFunction({
  ...
  args: [
    ...
    },
    {
      name: "code",
      description:
        "Code to execute for comparison (should return number: negative if first < second, 0 if equal, positive if first > second)",
      rest: false,
      type: ArgType.String,
      required: true,
    },
  ...
  async execute(ctx) {
    ...
    const var1Ret: Return = (await this["resolveCode"](
      ctx,
      var1Field,
    )) as Return;
    ...
    const var2Ret: Return = (await this["resolveCode"](
      ctx,
      var2Field,
    )) as Return;
    ...
    const sortedData = await asyncSort(
      store.data,
      async (x: string, y: string) => {
        ...
        const result: Return = (await this["resolveCode"](
          ctx,
          codeField,
        )) as Return;
